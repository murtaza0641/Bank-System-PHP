...
<p>The scope of this tool is to help us separate our structural queries into separate files for
better organization.</p>
<p>This tools job is to attempt to read all of those files and determine which ones are needed
to run against your database to synchronize the structure.</p>
<p>This tool only works for queries that take zero parameters.</p>
<p>It can be used to preload some data via inserts, but those queries <em>MUST</em> but
crafted in such a way that you don't generate duplicates during each run.</p>
<p>Files should be <a
href="https://en.wikipedia.org/wiki/Idempotence">Idempotent</a></p>
</details>
<br><br>
<?php
...
# Make sure you prefix the file names with at least a left padded 3 digit incrementing number
(i.e., 001, 002)
# This ensures the files always run in the precise order required (order of execution is extremely
important)
...
* Generates an array keyed by filename and valued as raw SQL from file* The .sql is important here since this directory may have other files types
*/
...
//to determine if they command should be skipped or not$line = $lines[0];
...
$line = trim($line);
if (in_array($line, $t)) {
echo "<p style=\"margin-left: 3em\">Blocked from running, table found in 'show
tables' results. [This is ok, it reduces redundant DB calls]</p><br>";
continue;
}
}
...
<summary>Status: <?php echo ($error[0] === "00000" ? "Success" : "Error");
?></summary>
...
echo "<p>Didn't find any files, please check the directory/directory contents/permissions
(note files must end in .sql)</p>";
}$db = null;
} catch (Exception $e) {
echo $e->getMessage();
